...
* **Trend C (Observation of the Formatting Difference Gap)**: By tracking the cosine subspace alignment between tasks with matching semantics but differing formats (SSSF vs. SSDF), we note a representation decoupling beginning in early layers (Layer 2, $\Delta = +0.0994$). This spatial difference maintains a relatively stable average margin—which we label the Formatting Difference Gap ($\Delta \approx 0.115$)—across intermediate layers (Layer 10 to Layer 25), as presented in Table 2. This represents an empirical statistical trend on our dataset rather than a deterministic, sample-independent physical law.
